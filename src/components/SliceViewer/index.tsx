...
import { FIELD_COLORMAP, getColorscale } from "../../lib/colormap";
...
export function SliceViewer({ result, selectedField }: Props) {
  ...
  const slice = useMemo<SliceData | null>(() => {
    ...
    // Find the center slice for the selected axis + field
    const match = result.slices.find(
      s => s.axis === axis && s.field === selectedField
    );
    // Fall back to any slice for this axis
    return match ?? result.slices.find(s => s.axis === axis) ?? result.slices[0] ?? null;
  ...
  if (!slice) {
    return <Placeholder text={`No ${axis}-axis slice in this result. Re-solve to include it.`} />;
  }

  const [rows, cols] = slice.shape;
  const z = [];
  for (let r = 0; r < rows; r++) {
    z.push(slice.data.slice(r * cols, (r + 1) * cols));
  }

  const colorscale = getColorscale(FIELD_COLORMAP[slice.field]);
  const axisPair = axis === "z" ? ["x", "y"] : axis === "x" ? ["y", "z"] : ["x", "z"];
  ...
  return (
    <div className="flex flex-col h-full bg-panel">
        ...
        {(["x", "y", "z"] as SliceAxis[]).map(a => (
          <button
            ...
            className={`px-3 py-0.5 rounded text-xs transition-colors
              ${axis === a ? "bg-accent/20 text-accent border border-accent/40" : "text-slate-500 hover:text-slate-300 border border-transparent"}`}
          >
            {a.toUpperCase()}
          </button>
        ))}
        <span className="ml-auto text-xs text-slate-500">
          {FIELD_LABELS[slice.field]}
      ...
      <div className="flex-1 min-h-0">
        <Plot
          data={[{
            ...
            colorscale,
            zmin: slice.field_min,
            zmax: slice.field_max,
            colorbar: {
              thickness: 12,
              len: 0.8,
          ...
          layout={{
            ...
            plot_bgcolor: "#09090d",
            xaxis: {
              title: { text: axisPair[0] + " (m)", font: { color: "#64748b", size: 10 } },
              tickfont: { color: "#64748b", size: 9 },
              gridcolor: "rgba(255,255,255,0.05)",
              zerolinecolor: "rgba(255,255,255,0.1)",
            },
            yaxis: {
              title: { text: axisPair[1] + " (m)", font: { color: "#64748b", size: 10 } },
              tickfont: { color: "#64748b", size: 9 },
              gridcolor: "rgba(255,255,255,0.05)",
              zerolinecolor: "rgba(255,255,255,0.1)",
              scaleanchor: "x",
      ...
      <div className="px-3 py-1 border-t border-rim text-xs text-slate-600 shrink-0 flex gap-4">
        ...
        {result.maxima.filter(m => m.field === slice.field).map(m => (
          <span key={m.field} className="text-slate-500">
            max {m.max_value.toExponential(3)} @ [{m.max_location.map(v => v.toFixed(3)).join(", ")}]
          </span>
        ))}
      </div>
    </div>
